...
import type { GitHubRepositoryAccess } from './setup.js';
...
export type GitHubCliToolOptions = {
...
export const githubCliToolDescription =
...
export const githubCliToolParameters = {
  type: 'object',
...
export async function executeGitHubCliTool(
...
async function readPreparedRepositoryBranch(repository: RepositoryToolServices): Promise<string> {
  ...
  if (!shell) throw new Error('gh pr create cannot infer --head before the sandbox shell is ready');
  const result = await shell('git branch --show-current', { cwd: prepared.workspacePath, timeoutMs: 30_000 });
...
async function readPreparedRepositoryCommit(
  ...
  const prepared = getPreparedRepository(repository);
  const shell = repository.shell();
  if (!shell) throw new Error('gh pr create cannot use --fill before the sandbox shell is ready');
  const result = await shell('git log -1 --pretty=format:%s%n%n%b', {
