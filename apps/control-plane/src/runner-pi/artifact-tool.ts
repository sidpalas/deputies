...
export function createPiArtifactToolDefinition(services: ArtifactToolServices): ToolDefinition {
  return {
    ...
    promptGuidelines: [
      ...
      'After user-visible UI changes, check browser capture with command -v deputies-record (or Playwright plus test -d "$PLAYWRIGHT_BROWSERS_PATH") and command -v ffmpeg. When Chromium is available, run the built app if practical, capture changed screens, and use read on each PNG to visually verify it before claiming success. On browserless sandboxes, skip capture or use the available fallback without failing the run.',
      'For multi-step flows, prefer deputies-record when available; otherwise record at 1280x720 for at most 60 seconds with Playwright recordVideo. Close the browser before transcoding, then use ffmpeg -i in.webm -c:v libx264 -pix_fmt yuv420p -movflags +faststart out.mp4 when ffmpeg exists.',
