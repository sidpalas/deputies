...
export class OpenComputerSandboxProvider implements SandboxProvider {
  ...
  async getServiceEndpoint(input: SandboxServiceEndpointInput): Promise<SandboxServiceEndpoint | null> {
    ...
    await ensureOpenComputerBridge(sandbox, this.workspacePath, bridgeToken, this.options.bridgeSkippedCookieNames);
    const hostname =
      sandbox.getPreviewDomain(sandboxBridgePort) ||
      (await createOpenComputerPreviewHostname(sandbox, sandboxBridgePort));
    if (!hostname) return null;
    return {
      port: input.port,
      targetUrl: sandboxBridgePreviewUrl(`https://${hostname}`, input.port),
  ...
  private toHandle(
    ...
    return {
      ...
      fs: createOpenComputerFileSystem(sandbox, this.workspacePath),
      exec: (command) =>
        execOpenComputerCommand(sandbox, command, {
          useProxyEgress: Boolean(this.options.secretStore),
          workspacePath: this.workspacePath,
        }),
    };
  }
...
function createOpenComputerFileSystem(sandbox: OpenComputerSandboxLike, workspacePath: string): SandboxFileSystem {
  return {
    readFile(path: string): Promise<string> {
      return sandbox.files.read(resolveOpenComputerPath(path, workspacePath));
    },
    readFileBuffer(path: string): Promise<Uint8Array> {
      return sandbox.files.readBytes(resolveOpenComputerPath(path, workspacePath));
    },
    writeFile(path: string, content: string | Uint8Array): Promise<void> {
      return sandbox.files.write(resolveOpenComputerPath(path, workspacePath), content);
    },
    async stat(path: string): Promise<FileStat> {
      const entry = await openComputerPathEntry(sandbox, resolveOpenComputerPath(path, workspacePath));
      return {
        isFile: !entry.isDir,
    ...
    async readdir(path: string): Promise<string[]> {
      const entries = await sandbox.files.list(resolveOpenComputerPath(path, workspacePath));
      return entries.map((entry) => entry.name).filter(Boolean);
    },
    async exists(path: string): Promise<boolean> {
      path = resolveOpenComputerPath(path, workspacePath);
      if (path === '/') return true;
    ...
    async mkdir(path: string, options?: { recursive?: boolean }): Promise<void> {
      const resolved = resolveOpenComputerPath(path, workspacePath);
      if (!options?.recursive) return sandbox.files.makeDir(resolved);
      const response = await runOpenComputerCommand(sandbox, `mkdir -p ${quoteShell(resolved)}`);
      if (response.exitCode !== 0) throw new Error(response.stderr || 'OpenComputer directory creation failed');
    },
    async rm(path: string, options?: { recursive?: boolean; force?: boolean }): Promise<void> {
      const resolved = resolveOpenComputerPath(path, workspacePath);
      try {
        if (!options?.recursive) await sandbox.files.remove(resolved);
        else {
          const flags = `${options.force ? 'f' : ''}r`;
          const response = await runOpenComputerCommand(sandbox, `rm -${flags} ${quoteShell(resolved)}`);
          if (response.exitCode !== 0) throw new Error(response.stderr || 'OpenComputer recursive removal failed');
        }
      } catch (error) {
...
async function execOpenComputerCommand(
  ...
  options: { useProxyEgress: boolean; workspacePath: string },
...
function openComputerRunOptions(
  input: SandboxExecInput,
  egress: { useProxyEgress: boolean; workspacePath: string },
): RunOpts {
  const runOptions: RunOpts = {};
  runOptions.cwd = resolveOpenComputerPath(input.cwd ?? '.', egress.workspacePath);
...
async function ensureOpenComputerBridge(
  ...
  const response = await runOpenComputerCommand(sandbox, sandboxBridgeStartupCommand(), {
    timeout: 10,
...
function resolveOpenComputerPath(path: string, workspacePath: string): string {
  ...
  if (workspaceRelative === '..' || workspaceRelative.startsWith('../') || isAbsolute(workspaceRelative))
    throw new Error(`OpenComputer path escapes workspace: ${path}`);
  return resolved;
}
