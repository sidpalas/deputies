...
describe('OpenComputerSandboxProvider', () => {
  ...
  it('creates an OpenComputer sandbox handle with exec and filesystem operations', async () => {
    ...
    expect(handle).toMatchObject({
      ...
      capabilities: { persistentFilesystem: true, exec: true, filesystem: true, previewUrls: true },
    });
  ...
  it('converts OpenComputer exec timeouts from milliseconds to seconds', async () => {
    ...
    expect(sandbox.execCalls).toEqual([{ command: `${clearProxyCommand}; sleep 5`, opts: { timeout: 3 } }]);
  });
  ...
  it('passes explicit command env to OpenComputer exec without adding parent env', async () => {
    ...
    expect(sandbox.execCalls).toEqual([
      { command: `${clearProxyCommand}; env | sort`, opts: { env: { GREETING: 'hello' } } },
    ]);
  });
  ...
  it('pipes stdin into OpenComputer exec commands', async () => {
    ...
    expect(sandbox.execCalls).toEqual([
      { command: `${clearProxyCommand}; printf %s 'hello '\\'' world' | cat > input.txt`, opts: {} },
    ]);
  });
  ...
  it('preserves explicit command proxy env when the caller supplies one', async () => {
    ...
    expect(sandbox.execCalls).toEqual([
      { command: 'curl https://example.com', opts: { env: { HTTPS_PROXY: 'http://proxy.example:3128' } } },
    ]);
  });
    ...
    expect(sandbox.execCalls).toEqual([
      {
        ...
        opts: { env: { GIT_SSL_CAINFO: '/usr/local/share/ca-certificates/opensandbox-proxy.crt' } },
      },
    ]);
  });
  ...
  it('rejects OpenComputer exec when the caller aborts while the SDK command is pending', async () => {
    ...
    expect(sandbox.execCalls).toEqual([{ command: `${clearProxyCommand}; sleep 20`, opts: {} }]);
  ...
  it('routes OpenComputer previews through an authenticated sandbox bridge preview', async () => {
    ...
    expect(sandbox.execCalls.at(-1)?.command).toContain('DEPUTIES_SANDBOX_BRIDGE_PORT=3584');
    expect(sandbox.execCalls.at(-1)?.command).toContain('DEPUTIES_SANDBOX_BRIDGE_PREVIEW_ONLY=1');
  });
