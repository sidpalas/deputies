...
describe.skipIf(!testDatabaseUrl)('control-plane load', () => {
  ...
  it('ingests generic webhook deliveries with dedupe under concurrent load', async () => {
    ...
    const [messageCounts, sessionCounts, deliveryCounts] = await Promise.all([countByStatus(pool, 'messages'), countByStatus(pool, 'sessions'), countByStatus(pool, 'integration_deliveries')]);
    await writeLoadSummary('generic_webhook_ingestion', { deliveryCount: webhookDeliveryCount, concurrency: webhookConcurrency, elapsedSeconds, deliveriesPerSecond: rate(webhookDeliveryCount, elapsedSeconds), acceptedCount, duplicateCount, messageCounts, sessionCounts, deliveryCounts });
...
function createWorker(input: { store: PostgresStore; events: EventService; sandboxProvider: SandboxProvider; leaseOwner: string; runner?: Runner }): WorkerService {
  return new WorkerService({ store: input.store, events: input.events, runner: input.runner ?? new FakeRunner(), runnerType: 'fake', sandboxProvider: input.sandboxProvider, leaseOwner: input.leaseOwner, leaseDurationMs: 60_000, heartbeatIntervalMs: 30_000, cancellationPollIntervalMs: 30_000 });
}
...
class MethodProfiler {
  ...
  get totalCount(): number { return Array.from(this.stats.values()).reduce((sum, item) => sum + item.count, 0); }
  async measure<T>(name: string, fn: () => Promise<T>): Promise<T> {
    const startedAt = performance.now();
    try { return await fn(); } finally { this.record(name, performance.now() - startedAt); }
  }
  record(name: string, elapsedMs: number): void {
    const existing = this.stats.get(name) ?? { count: 0, totalMs: 0, maxMs: 0 };
    existing.count += 1; existing.totalMs += elapsedMs; existing.maxMs = Math.max(existing.maxMs, elapsedMs); this.stats.set(name, existing);
  }
  snapshot(): Record<string, MethodStats> { return Object.fromEntries(Array.from(this.stats.entries()).map(([name, item]) => [name, { ...item }])); }
  summary(options: { top?: number } = {}): Record<string, { count: number; totalMs: number; avgMs: number; maxMs: number }> {
    return Object.fromEntries(Array.from(this.stats.entries()).sort(([, l], [, r]) => r.totalMs - l.totalMs).slice(0, options.top ?? Number.POSITIVE_INFINITY).map(([name, item]) => [name, { count: item.count, totalMs: round(item.totalMs, 3), avgMs: round(item.totalMs / item.count, 3), maxMs: round(item.maxMs, 3) }]));
  }
}

function profileObject<T extends object>(target: T, profiler: MethodProfiler): T {
  return new Proxy(target, { get(object, property, receiver) { const value = Reflect.get(object, property, receiver) as unknown; if (typeof value !== 'function' || typeof property !== 'string') return value; return (...args: unknown[]) => profiler.measure(property, () => Promise.resolve(value.apply(object, args))); } });
}
...
class ProfilingRunner implements Runner {
  ...
  async run(input: RunnerInput): Promise<RunnerResult> { return this.profiler.measure('run', () => this.delegate.run(input)); }
}

class ProfilingSandboxProvider implements SandboxProvider {
  readonly name: string;
  readonly capabilities: SandboxCapabilities;
  constructor(private readonly delegate: SandboxProvider, private readonly profiler: MethodProfiler) { this.name = delegate.name; this.capabilities = delegate.capabilities; }
  create(input: CreateSandboxInput): Promise<SandboxHandle> { return this.profiler.measure('create', () => this.delegate.create(input)); }
  connect(input: ConnectSandboxInput): Promise<SandboxHandle> { return this.profiler.measure('connect', () => this.delegate.connect(input)); }
  start(input: SandboxRef): Promise<void> { return this.delegate.start ? this.profiler.measure('start', () => this.delegate.start!(input)) : Promise.resolve(); }
  stop(input: SandboxRef): Promise<void> { return this.delegate.stop ? this.profiler.measure('stop', () => this.delegate.stop!(input)) : Promise.resolve(); }
  destroy(input: SandboxRef): Promise<void> { return this.profiler.measure('destroy', () => this.delegate.destroy(input)); }
  health(input: SandboxRef): Promise<SandboxHealth> { return this.profiler.measure('health', () => this.delegate.health(input)); }
}

async function truncateAppTables(pool: Pool): Promise<void> {
  await pool.query('TRUNCATE flue_sessions, callback_deliveries, artifacts, integration_deliveries, external_threads, sandboxes, events, runs, messages, session_sequence_counters, webhook_sources, auth_sessions, auth_accounts, auth_users, sessions RESTART IDENTITY CASCADE');
}

async function seedPendingBacklog(pool: Pool, input: { sessionCount: number; messagesPerSession: number }): Promise<void> {
  await pool.query(`INSERT INTO sessions (id, status, title, created_at, updated_at) SELECT session_id, 'idle', 'Load session ' || session_index, now(), now() FROM generate_series(1, $1::int) AS session_index CROSS JOIN LATERAL (SELECT ('00000000-0000-4000-8000-' || lpad(to_hex(session_index), 12, '0'))::uuid AS session_id) ids`, [input.sessionCount]);
  await pool.query(`INSERT INTO messages (id, session_id, sequence, status, prompt, source, context, created_at) SELECT ('00000000-0000-4000-9000-' || lpad(to_hex(((session_index - 1) * $2::int) + message_sequence), 12, '0'))::uuid, ('00000000-0000-4000-8000-' || lpad(to_hex(session_index), 12, '0'))::uuid, message_sequence, 'pending', 'Load prompt ' || session_index || '.' || message_sequence, 'load-test', '{}'::jsonb, now() + ((((session_index - 1) * $2::int) + message_sequence) || ' microseconds')::interval FROM generate_series(1, $1::int) AS session_index CROSS JOIN generate_series(1, $2::int) AS message_sequence`, [input.sessionCount, input.messagesPerSession]);
}

async function seedContentionBacklog(pool: Pool, input: { sessionCount: number }): Promise<void> {
  await pool.query(`INSERT INTO sessions (id, status, title, queue_paused_at, created_at, updated_at) SELECT ('00000000-0000-4000-a000-' || lpad(to_hex(session_index), 12, '0'))::uuid, CASE WHEN session_index % 17 = 0 THEN 'archived' ELSE 'queued' END, 'Contention session ' || session_index, CASE WHEN session_index % 13 = 0 THEN now() ELSE NULL END, now() - (session_index || ' seconds')::interval, now() - (session_index || ' seconds')::interval FROM generate_series(1, $1::int) AS session_index`, [input.sessionCount]);
  await pool.query(`INSERT INTO messages (id, session_id, sequence, status, prompt, source, context, created_at) SELECT ('00000000-0000-4000-a100-' || lpad(to_hex(session_index), 12, '0'))::uuid, ('00000000-0000-4000-a000-' || lpad(to_hex(session_index), 12, '0'))::uuid, 1, 'pending', 'Contention prompt ' || session_index, 'load-contention', '{}'::jsonb, now() - (($1::int - session_index) || ' milliseconds')::interval FROM generate_series(1, $1::int) AS session_index`, [input.sessionCount]);
  await pool.query(`INSERT INTO runs (id, session_id, message_id, status, runner_type, lease_owner, lease_expires_at, heartbeat_at, started_at, metadata) SELECT ('00000000-0000-4000-a200-' || lpad(to_hex(session_index), 12, '0'))::uuid, ('00000000-0000-4000-a000-' || lpad(to_hex(session_index), 12, '0'))::uuid, ('00000000-0000-4000-a100-' || lpad(to_hex(session_index), 12, '0'))::uuid, 'running', 'fake', 'existing-worker', now() + interval '1 minute', now(), now(), '{}'::jsonb FROM generate_series(1, $1::int) AS session_index WHERE session_index % 19 = 0`, [input.sessionCount]);
}

async function countClaimableContentionSessions(pool: Pool): Promise<number> {
  const result = await pool.query<{ count: string }>(`SELECT count(*)::text AS count FROM messages m JOIN sessions s ON s.id = m.session_id WHERE m.status = 'pending' AND s.status <> 'archived' AND s.queue_paused_at IS NULL AND NOT EXISTS (SELECT 1 FROM runs r WHERE r.session_id = m.session_id AND r.status IN ('starting', 'running', 'cancelling') AND (r.lease_expires_at IS NULL OR r.lease_expires_at > now()))`);
...
async function seedReadHeavyDataset(pool: Pool, input: { sessionCount: number; eventsPerSession: number; hotSessionEvents: number }): Promise<void> {
  const hotSessionId = uuidFromInt('8100', 1);
  await pool.query(`INSERT INTO sessions (id, status, title, context, created_at, updated_at) VALUES ($1, 'idle', 'Hot event session', '{"kind":"hot"}'::jsonb, now() - interval '7 days', now())`, [hotSessionId]);
  await pool.query(`INSERT INTO sessions (id, status, title, context, queue_paused_at, created_at, updated_at) SELECT ('00000000-0000-4000-8200-' || lpad(to_hex(session_index), 12, '0'))::uuid, CASE WHEN session_index % 29 = 0 THEN 'archived' WHEN session_index % 23 = 0 THEN 'active' WHEN session_index % 19 = 0 THEN 'failed' WHEN session_index % 17 = 0 THEN 'queued' ELSE 'idle' END, 'Read session ' || session_index, jsonb_build_object('index', session_index, 'source', CASE WHEN session_index % 2 = 0 THEN 'github' ELSE 'slack' END), CASE WHEN session_index % 31 = 0 THEN now() - interval '1 hour' ELSE NULL END, now() - (session_index || ' minutes')::interval, now() - (session_index || ' seconds')::interval FROM generate_series(1, $1::int) AS session_index`, [input.sessionCount]);
  await pool.query(`INSERT INTO events (session_id, run_id, message_id, sequence, type, payload, created_at) SELECT ('00000000-0000-4000-8200-' || lpad(to_hex(session_index), 12, '0'))::uuid, NULL, NULL, event_sequence, CASE WHEN event_sequence % 5 = 0 THEN 'agent_text_delta' ELSE 'run_started' END, jsonb_build_object('index', event_sequence, 'text', repeat('event payload ', CASE WHEN event_sequence % 11 = 0 THEN 20 ELSE 1 END)), now() - (session_index || ' seconds')::interval + (event_sequence || ' milliseconds')::interval FROM generate_series(1, $1::int) AS session_index CROSS JOIN generate_series(1, $2::int) AS event_sequence`, [input.sessionCount, input.eventsPerSession]);
  await pool.query(`INSERT INTO events (session_id, run_id, message_id, sequence, type, payload, created_at) SELECT $1::uuid, NULL, NULL, event_sequence, CASE WHEN event_sequence % 3 = 0 THEN 'agent_text_delta' ELSE 'run_completed' END, jsonb_build_object('index', event_sequence, 'text', repeat('hot event payload ', CASE WHEN event_sequence % 7 = 0 THEN 50 ELSE 2 END)), now() - interval '1 day' + (event_sequence || ' milliseconds')::interval FROM generate_series(1, $2::int) AS event_sequence`, [hotSessionId, input.hotSessionEvents]);
}

async function seedWebhookSource(pool: Pool): Promise<void> {
  await pool.query(`INSERT INTO webhook_sources (id, key, name, enabled, bearer_token, prompt_prefix, created_at, updated_at) VALUES ('00000000-0000-4000-b000-000000000001'::uuid, 'load-generic', 'Load Generic', true, 'load-token', 'Load prefix', now(), now())`);
}

async function runConcurrent<T>(count: number, concurrency: number, task: (index: number) => Promise<T>): Promise<T[]> {
  const results = new Array<T>(count); let nextIndex = 0;
  await Promise.all(new Array(Math.min(count, concurrency)).fill(0).map(async () => { while (true) { const index = nextIndex; nextIndex += 1; if (index >= count) return; results[index] = await task(index); } }));
  return results;
}
...
async function countRows(pool: Pool, table: 'events' | 'sandboxes'): Promise<number> { const result = await pool.query<{ count: string }>(`SELECT count(*)::text AS count FROM ${table}`); return Number(result.rows[0]!.count); }
async function measureAsync<T>(fn: () => Promise<T>): Promise<{ value: T; elapsedMs: number }> { const startedAt = performance.now(); const value = await fn(); return { value, elapsedMs: round(performance.now() - startedAt, 3) }; }
function summarizeMeasurement<T>(measurement: { value: T; elapsedMs: number }): { elapsedMs: number; rows?: number } { return { elapsedMs: measurement.elapsedMs, ...(Array.isArray(measurement.value) ? { rows: measurement.value.length } : {}) }; }
async function writeLoadSummary(name: string, metrics: Record<string, unknown>): Promise<Record<string, unknown>> { const summary = { name, timestamp: new Date().toISOString(), ...metrics }; const line = JSON.stringify(summary); process.stdout.write(`\nload summary: ${line}\n`); if (reportPath) await appendFile(reportPath, `${line}\n`, 'utf8'); return summary; }
function secondsSince(startedAt: number): number { return round((performance.now() - startedAt) / 1_000, 3); }
function rate(count: number, seconds: number): number { return round(count / Math.max(seconds, 0.001), 1); }
function round(value: number, decimals: number): number { const factor = 10 ** decimals; return Math.round(value * factor) / factor; }
function uuidFromInt(prefix: string, value: number): string { return `00000000-0000-4000-${prefix}-${value.toString(16).padStart(12, '0')}`; }
function readPositiveIntEnv(name: string, fallback: number): number { const parsed = readPositiveNumberEnv(name, fallback); if (!Number.isInteger(parsed)) throw new Error(`${name} must be an integer`); return parsed; }
function readPositiveNumberEnv(name: string, fallback: number): number { const raw = process.env[name]; if (!raw) return fallback; const parsed = Number(raw); if (!Number.isFinite(parsed) || parsed <= 0) throw new Error(`${name} must be a positive number`); return parsed; }
