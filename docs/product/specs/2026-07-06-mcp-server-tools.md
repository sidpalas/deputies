...
- **Pi runner** (`src/runner-pi/`, primary going forward) — builds a `customTools: ToolDefinition[]` array per run in `createPiToolSet` (`src/runner-pi/runner.ts:309`) and passes it to `createAgentSession({ noTools: 'builtin', customTools })`. Pi (`@earendil-works/pi-coding-agent` 0.78.0) has **no MCP support**.
