...
The runner resolves tenant-wide skills after repository preparation and before the first prompt. Managed auto-load candidates follow each enabled, non-archived skill's current revision at run start. Manual invocations resolve separately for each claimed message.

The API, not the browser, pins a new manual managed invocation. On enqueue it authorizes the current tenant or personal skill for the message actor and persists that revision. A client-supplied old revision is rejected. At execution, a persisted historical pin loads only while the skill remains enabled, non-archived, and still authorized. Repository refs remain revisionless.
...
Prepared repositories are scanned in `.agents/skills/`, `.claude/skills/`, then `.pi/skills/` order. The runner mirrors bounded discovery data through the worker and rewrites paths back to the sandbox. Tenant managed skills take precedence over same-named repository skills.

Repository discovery records advertised, shadowed, and `disable-model-invocation` skills in `skills_loaded`. `GET /sessions/:sessionId/skills` and integration slash lookup use the latest event as their repository discovery index. `GET /skills/invocation-candidates` returns tenant managed skills without requiring a sandbox.
