...
For static session auth, include `AUTH_STATIC_USERNAME` and `AUTH_STATIC_PASSWORD` in the referenced Secret. For GitHub session auth, set `config.apiAuthMode=session`, `config.authProvider=github`, `secrets.githubOAuthClientId`, `secrets.githubOAuthClientSecret`, and one or more access controls such as `config.authGithubAdminUsers`, `config.authGithubAllowedUsers`, or `config.authGithubAllowedOrganizations`. `config.authGithubDefaultRole` controls the tenant role for non-admin GitHub users, and `config.unsafeAuthGithubAllowAll=true` is only intended for public trial access.
