...
function createImage(args: Args): Image {
  ...
  image = image
    ...
    .runCommands('chmod +x /opt/deputies/ensure-sandbox-bridge.sh');
...
function createRunCommands(args: Args): string[] {
  ...
  commands.push(createWorkspaceCommand());
...
function parseArgs(values: string[], env: NodeJS.ProcessEnv): Args {
  let nameProvided = Boolean(env.OPENCOMPUTER_SNAPSHOT_NAME || env.OPENCOMPUTER_SNAPSHOT);
  ...
  const args: Args = {
    ...
    name: env.OPENCOMPUTER_SNAPSHOT_NAME ?? env.OPENCOMPUTER_SNAPSHOT ?? defaultSnapshotName(variant),
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    if (value === '--help' || value === '-h') {
      ...
      if (!nameProvided) args.name = defaultSnapshotName(args.variant);
...
function defaultSnapshotName(variant: SnapshotVariant): string {
  if (variant === 'minimal') return defaultMinimalSnapshotName;
  if (variant === 'bridge') return defaultBridgeSnapshotName;
  return variant === 'slim' ? defaultSlimSnapshotName : defaultFullSnapshotName;
}
...
Options:
  --name <name>                 Snapshot name. Defaults to OPENCOMPUTER_SNAPSHOT_NAME,
                                OPENCOMPUTER_SNAPSHOT, or the variant default.
...
Defaults:
  full snapshot name: ${defaultFullSnapshotName}
  slim snapshot name: ${defaultSlimSnapshotName}
  bridge snapshot name: ${defaultBridgeSnapshotName}
  minimal snapshot name: ${defaultMinimalSnapshotName}
